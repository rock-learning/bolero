...
class CartesianDMPBehavior(BlackBoxBehavior):
    ...
    def load_config(self, filename):
        ...
        self.x0 = np.array(config["startPosition"], dtype=float)
        self.x0d = np.array(config["startVelocity"], dtype=float)
        self.x0dd = np.array(config["startAcceleration"], dtype=float)
        self.q0 = np.array(config["startRotation"], dtype=float)
        self.q0d = np.array(config["startAngularVelocity"], dtype=float)
        self.g = np.array(config["endPosition"], dtype=float)
        self.gd = np.array(config["endVelocity"], dtype=float)
        self.gdd = np.array(config["endAcceleration"], dtype=float)
        self.qg = np.array(config["endRotation"], dtype=float)
