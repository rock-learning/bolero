...
/* --- Changes : --- 
  ...
  06/02/25: Overflow in time measurement for runs longer than 
            2100 seconds corrected. They could lead to stalling the 
            covariance matrix update for long periods. 
  06/02/26: Include population size lambda as parameter to 
            cmaes_init (thanks to MT). 
  06/02/26: Allow no initial reading/writing of parameters via
            "non" and "nowrite" keywords for input parameter 
...
/* see cmaes_interface.h for all other remaining, not listed here */

void   random_init( random_t *, long unsigned seed /*= -1*/);
void   random_exit( random_t *);
double random_Gauss( random_t *); /* (0,1)-normally distributed */
double random_Uniform( random_t *);
void   random_Start( random_t *, long unsigned seed /*= -1*/);
...
void readpara_init (readpara_t *, int dim, int seed,  const double * xstart, 
		    const double * rgcoordist, int lambda, const char * filename);
		...
		const char *input_parameter_filename) 
{
  ...
  double dtest, trace;
  t->pFitFunc = pFun;
  readpara_init (&t->sp, dimension, inseed, inxstart, inrgsigma, 
  		 lambda, input_parameter_filename);
  random_init( &t->rand, (unsigned) t->sp.seed);
...
{ 
  ...
  do /* while *key=='+' */
	...
	{
	  timings_update(&t->timings);
	  fprintf(fp, "%.1f %.1f",  t->timings.totaltime, 
		  t->timings.tictoctime);
...
*/
{
  ...
  static time_t writetime = 0; /* ideally should keep track for each output file */
...
cmaes_UpdateEigensystem(cmaes_t *t, int flgforce)
{
  ...
  if(flgforce == 0) {
    ...
    if (t->sp.updateCmode.maxtime < 100 
	&& t->timings.tictoctime > 0.01 * t->sp.updateCmode.maxtime * t->timings.totaltime
	&& t->timings.tictoctime > 0.0002)
      return; 
  }
  timings_tic(&t->timings);
  ...
  for (iEigenCalcVers = 0; iEigenCalcVers < maxEigenCalcVers; 
      ...
      if (cIterEig < maxIterEig) 
	{
	  ...
	  timings_toc(&t->timings);
	  return;
	} /* if cIterEig < ... */
      
      /* Eigenzerlegung numerische nicht OK */
      {
	char *s=(char*)new_void(160+40*N,1); s[0] = '\0'; /* MT */
	...
	free(s); /* MT */
      ...
      summand = t->dLastMinEWgroesserNull * exp((double)iEigenCalcVers);
      for (i = 0; i < N; ++i)
	t->C[i][i] += summand;
 ...
 */
  double diffc, difft;
  clock_t lc = t->lastclock;
  time_t lt = t->lasttime;
...
random_init( random_t *t, long unsigned inseed)
{
  ...
  if (inseed < 1) {
    ...
    inseed = (long)abs(100*time(NULL)+(clock()*getpid()));
  }
...
/* --------------------------------------------------------- */
void random_Start( random_t *t, long unsigned inseed)
{
  long tmp;
  int i;
	       ...
	       const char * filename)
{
  ...
  /* All scalars:  */
  t->rgsformat[0] = " N %d";        t->rgpadr[0] = (void *) &t->N;
  t->rgsformat[1] = " seed %d";    t->rgpadr[1] = (void *) &t->seed;
  t->rgsformat[2] = " maxeval %lg"; t->rgpadr[2] = (void *) &t->maxeval;
  t->rgsformat[3] = " stopfunval %lg"; t->rgpadr[3]=(void *) &t->stStopFuncValue.val;
  t->rgsformat[4] = " stopfunvaldiff %lg"; t->rgpadr[4]=(void *) &t->stopFuncValDiff;
  t->rgsformat[5] = " lambda %d";      t->rgpadr[5] = (void *) &t->lambda;
  t->rgsformat[6] = " mu %d";          t->rgpadr[6] = (void *) &t->mu;
  t->rgsformat[7] = " weights %5s";    t->rgpadr[7] = (void *) t->weigkey;
  t->rgsformat[8] = " fac*ccumsig %lg";t->rgpadr[8] = (void *) &t->ccumsig;
  t->rgsformat[9] = " fac*damp %lg";   t->rgpadr[9] = (void *) &t->damp;
  t->rgsformat[10] = " ccumcov %lg";    t->rgpadr[10] = (void *) &t->ccumcov;
  t->rgsformat[11] = " mucov %lg";     t->rgpadr[11] = (void *) &t->mucov;
  t->rgsformat[12] = " fac*ccov %lg";  t->rgpadr[12]=(void *) &t->ccov;
  t->rgsformat[13] = " updatecov %lg"; t->rgpadr[13]=(void *) &t->updateCmode.modulo;
  t->rgsformat[14] = " updatecovmaxtime %lg"; t->rgpadr[14]=(void *) &t->updateCmode.maxtime;
  t->rgsformat[15] = " resume %59s";    t->rgpadr[15] = (void *) t->resumefile;
  t->rgsformat[16] = " fac*maxeval %lg";   t->rgpadr[16] = (void *) &t->facmaxeval;
  t->rgsformat[17] = " fac*updatecov %lg"; t->rgpadr[17]=(void *) &t->facupdateCmode;
  t->n1para = 18; 
  t->n1outpara = 16; /* disregard last parameters in WriteToFile() */

  /* arrays */
  t->rgskeyar[0]  = " xstart %d";   t->rgp2adr[0] = &t->xstart;
  t->rgskeyar[1]  = " sigma %d"; t->rgp2adr[1] = &t->rgcoordist;
  t->rgskeyar[2]  = " mincoorstddev %d"; t->rgp2adr[2] = &t->rgMinCoorStddev;
  t->n2para = 3;  
  ...
  if (t->rgcoordist == NULL && inrgsigma == NULL) {
  ...
  if (t->rgcoordist == NULL) {
    ...
    for (i=0; i<N; ++i)
      t->rgcoordist[i] = (inrgsigma == NULL) ? 0.3 : inrgsigma[i];
  }
