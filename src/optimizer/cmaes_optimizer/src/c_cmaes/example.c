...
int optimize(double(*pFun)(double *), char *input_parameter_filename);
...
int main(int argn, char **args)
{
  ...
  char *filename = "incmaes.par"; /* input parameter file */
  FILE *fp = NULL;
  int nb = 0;
  int maxnb;
  ...
  /* Read objective function number from file */
  fp = fopen(filename, "r");
  if (fp) {
...
int optimize(double(*pFun)(double *), char * filename)
{
  cmaes_t evo; 
  double *rgFunVal = NULL;
  double *const*rgx; /* pointer to the sampled population */

  cmaes_init(&evo, pFun, 0, NULL, NULL, 0, 0, filename);
...
void writeoutput(cmaes_t *pevo)
{
  ...
  if (0 && (fmod(gen, outmodwrite) < 1 
    ...
    outtime += (double)(clock()-lasttime0)/CLOCKS_PER_SEC; /* might not work properly? */
  }
