...
void PyBehaviorSearch::init(int numInputs, int numOutputs) {
  ...
  PyObject *result;
  result = PyObject_CallMethod(py_behaviorSearch.get(), (char*)"init",
                               (char*)"ii", numInputs, numOutputs);
  if(!result) {
    PyErr_Print();
  }
  Helper::instance().createCallableInfo(&set_evaluation_feedback, py_behaviorSearch.get(),
...
void PyBehaviorSearch::setEvaluationFeedback(const double *feedbacks,
                                              int numFeedbacks) {
  Helper::instance().fillCallableInfo(&set_evaluation_feedback, feedbacks, numFeedbacks);
  PyObject *pResult = PyObject_CallObject(set_evaluation_feedback.callable,
                                          set_evaluation_feedback.argTuple);
  if(pResult) {
    Py_DECREF(pResult);
  } else {
    PyErr_Print();
...
void PyBehaviorSearch::writeResults(const std::string &resultPath) {
  assert(py_behaviorSearch);
  PyObject *pResult;
  pResult = PyObject_CallMethod(py_behaviorSearch.get(), (char*)"write_results",
...
bool PyBehaviorSearch::isBehaviorLearningDone() const {
  ...
  PyObject *result;
  bool ret = false;
  result = PyObject_CallMethod(py_behaviorSearch.get(),
