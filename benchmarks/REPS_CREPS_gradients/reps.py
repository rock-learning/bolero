...
from bolero.representation.ul_policies import ConstantGaussianPolicy
from bolero.utils.validation import check_random_state, check_feedback
from bolero.utils.log import get_logger

def solve_dual_reps(R, epsilon, min_eta, approx_grad = False):
    ...
    def g_grad(eta):  # Objective function with gradient
        R_over_eta = R / eta
...
class REPSOptimizer(Optimizer):
    ...
    def __init__(self, initial_params=None, variance=1.0, covariance=None,
                 ...
                 log_to_stdout=False, random_state=None, approx_grad = False):
    ...
    def set_evaluation_feedback(self, feedbacks):
        ...
        if self.it % self.train_freq == 0:
            ...
            d = solve_dual_reps(R, self.epsilon, self.min_eta, self.approx_grad)[0]
