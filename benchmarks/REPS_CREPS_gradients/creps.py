...
from bolero.representation.ul_policies import (ContextTransformationPolicy,
                                          LinearGaussianPolicy,
                                          BoundedScalingPolicy)
from bolero.utils.validation import check_random_state, check_feedback, check_context
...
def solve_dual_contextual_reps(S, R, epsilon, min_eta, approx_grad = True):
    ...
    def g_grad(x):  # Objective function
        eta = x[0]
        nu = x[1:]
...
class CREPSOptimizer(ContextualOptimizer):
    ...
    def __init__(self, initial_params=None, variance=None, covariance=None,
                 ...
                 random_state=None, approx_grad = True, **kwargs):
